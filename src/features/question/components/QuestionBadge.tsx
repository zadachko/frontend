...
const QuestionBadge = ({
    ...
    return (
        <div className="flex-shrink-0 flex flex-col items-center gap-2">
            <div
                className={`${isSmallMobile ? 'w-6 h-6 text-xs' : isMobile ? 'w-8 h-8 text-xs' : 'w-10 h-10 text-sm'} rounded-full flex items-center justify-center font-semibold bg-gray-100 text-gray-600`}
            >
                {questionNumber}
            </div>
            {onCircleClick && showSolutionButton && (
                <div
