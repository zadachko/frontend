...
const OpenAnswer = ({
    ...
    if (isReviewMode) {
        ...
        return (
            <div className="space-y-3">
                ...
                >
                    <span className={`${isSmallMobile ? 'text-xs' : isMobile ? 'text-xs' : 'text-sm'} text-gray-600`}>
                        Вашият отговор:{' '}
                    </span>
                    <span
                        className={`${isSmallMobile ? 'text-xs' : isMobile ? 'text-xs' : 'text-sm'} font-medium ${
                            isCorrect
                                ? 'text-green-800'
                                : userAnswer && userAnswer !== correctAnswer
                                  ? 'text-red-800'
                ...
                {userAnswer && userAnswer !== correctAnswer && (
                    <div className="p-3 rounded border bg-green-50 border-green-200">
                        <span
                            className={`${isSmallMobile ? 'text-xs' : isMobile ? 'text-xs' : 'text-sm'} text-gray-600`}
                        >
                            Правилен отговор:{' '}
                        </span>
                        <span
                            className={`${isSmallMobile ? 'text-xs' : isMobile ? 'text-xs' : 'text-sm'} font-medium text-green-800`}
                        >
                            {correctAnswer}
                        </span>
                    </div>
    ...
    return (
        <div className={`${isMobile ? 'w-full' : 'max-w-md'}`}>
            <label
                className={`block ${isSmallMobile ? 'text-xs' : isMobile ? 'text-xs' : 'text-sm'} font-medium text-gray-700 mb-2`}
            >
                Твоят отговор:
            </label>
            <Input
