        ...
        <div className={`${isMobile ? 'flex-1 p-4 max-w-80 m-auto' : 'flex-1 p-6'} ${isSmallMobile ? 'p-2' : ''}`}>
            ...
            {isMobile && (
                <div className="flex items-center justify-between mb-4 pb-3 border-b border-gray-200">
                    <h3
                        className={`font-semibold text-gray-900 flex items-center gap-2 ${isSmallMobile ? 'text-sm' : ''}`}
                    >
                        <Flag
                            className={`${isSmallMobile ? 'w-3 h-3' : 'w-4 h-4'} ${navigatorColors.primary.startsWith('[') ? `text-${navigatorColors.primary}` : `text-${navigatorColors.primary}-600`}`}
                        />
                        Въпроси
                    </h3>
                </div>
            ...
            {!isMobile && (
                <div className="mb-6">
                    <h3 className="font-semibold text-gray-900 mb-2 flex items-center gap-2">
                        <Flag
                            className={`w-4 h-4 ${navigatorColors.primary.startsWith('[') ? `text-${navigatorColors.primary}` : `text-${navigatorColors.primary}-600`}`}
                        />
                        Въпроси
                    </h3>
            ...
            <div
                className={`grid gap-2 mb-6 m-auto ${isSmallMobile ? 'grid-cols-5 gap-0.5 w-fit' : isMobile ? 'grid-cols-5 gap-0.5' : 'grid-cols-5 gap-0.5'}`}
            >
                {Array.from({ length: totalQuestions }, (_, i) => i + 1).map((questionNum) => {
                    ...
                    return (
                        <button
                            ...
                            className={`${isSmallMobile ? 'w-12 h-12 text-xs' : isMobile ? 'w-12 h-12 text-xs' : 'w-12 h-12 text-sm'} rounded-lg font-semibold transition-all duration-200 border-2 ${
                                isCurrent
            ...
            {/* Legend */}
            <div
                className={`space-y-3 text-sm mb-6 ${isSmallMobile ? 'text-xs space-y-2' : isMobile ? 'text-xs' : ''}`}
            >
                {!reviewMode && (
                    <div className="flex items-center gap-3">
                        <div
                            className={`${isSmallMobile ? 'w-3 h-3' : 'w-4 h-4'} rounded bg-${navigatorColors.primary}`}
                        ></div>
                ...
                {!reviewMode && (
                    <div className="flex items-center gap-3">
                        <div
                            className={`${isSmallMobile ? 'w-3 h-3' : 'w-4 h-4'} rounded bg-${navigatorColors.answeredBg} border-2 border-${navigatorColors.answeredBorder} text-${navigatorColors.answeredText}`}
                        ></div>
                ...
                {reviewMode && (
                    <div className="flex items-center gap-3">
                        <div
                            className={`${isSmallMobile ? 'w-3 h-3' : 'w-4 h-4'} rounded bg-green-100 border-2 border-green-500`}
                        ></div>
                ...
                {reviewMode && (
                    <div className="flex items-center gap-3">
                        <div
                            className={`${isSmallMobile ? 'w-3 h-3' : 'w-4 h-4'} rounded bg-red-100 border-2 border-red-500`}
                        ></div>
                ...
                {!reviewMode && (
                    <div className="flex items-center gap-3">
                        <div
                            className={`${isSmallMobile ? 'w-3 h-3' : 'w-4 h-4'} rounded bg-gray-50 border-2 border-gray-200`}
                        ></div>
